...
std::optional<std::string> lgr_get(const std::string& lgr_name) {
    easy_handle curl = easy_handle(sequential_handle);
    curl.setopt(CURLOPT_URL, std::format(HOST_URL "lgr/get/{}" LGR_DL, lgr_name).c_str());
...
// https://api.elma.online/api/lgr/get/LGRNAME.lgr?dl
void lgr_get_multi(std::vector<std::string> lgr_names) {
    ...
    for (const std::string& lgr_name : lgr_names) {
        ...
        handle->setopt(CURLOPT_URL, std::format(HOST_URL "lgr/get/{}" LGR_DL, lgr_name).c_str());
